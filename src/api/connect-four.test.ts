...
describe('Initializing Game', () => {
    ...
    test('board should be of correct size', () => {
        expect(connectFour.board.length).toBe(ConnectFour.BOARD_ROWS);
        expect(connectFour.board[0]!.length).toBe(ConnectFour.BOARD_COLS);
    });
...
describe('Gameplay', () => {
    ...
    test('should get the coordinate where it was placed, be allowed to make a move and change player', () => {
        expect(connectFour.currentPlayer).toBe(Player.ONE);
        expect(connectFour.makeMove(0)).toEqual({ x: 0, y: ConnectFour.BOARD_ROWS - 1 });
        expect(connectFour.currentPlayer).toBe(Player.TWO);
    });

    test('second player should be able to make a move and change player', () => {
        expect(connectFour.currentPlayer).toBe(Player.TWO);
        expect(connectFour.makeMove(0)).toEqual({ x: 0, y: ConnectFour.BOARD_ROWS - 2 });
        expect(connectFour.currentPlayer).toBe(Player.ONE);
    });

    test('should be able play one after another', () => {
        for (let i = 1; i < ConnectFour.BOARD_ROWS; i++) {
            expect(connectFour.makeMove(i)).toEqual({ x: i, y: ConnectFour.BOARD_ROWS - 1 });
        }
    });
    ...
    test('should fail after filling up', () => {
        const x = 1;
        for (let i = 1; i < ConnectFour.BOARD_ROWS; i++) {
            expect(connectFour.makeMove(x)).toEqual({ x, y: ConnectFour.BOARD_ROWS - i - 1 });
        }
        expect(() => connectFour.makeMove(x)).toThrowError(ConnectFourError.ALREADY_TAKEN);
    });

    test('should fail after illegal move', () => {
        expect(() => connectFour.makeMove(ConnectFour.BOARD_COLS)).toThrowError(ConnectFourError.OUT_OF_BOUNDS);
